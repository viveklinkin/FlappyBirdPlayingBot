...
def initialize_empty_qvals():
    ...
    for x in list(range(-40,491,10)):
        for y in list(range(-300,421,10)):
            for vel in range(-10,11,1):
...
try:
    ...
    initialize_empty_qvals()

def get_state_key(x, y, vel):
    return str(round_off(x)) + '_' + str(round_off(y)) + '_' + str(vel)
